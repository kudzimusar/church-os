...
async function getBroadcastHistory() {
  ...
  const history = broadcasts?.map(bc => {
    const bcStats = stats?.filter(s => s.broadcast_id === bc.id) || [];
    const total = bcStats.length;
    const read = bcStats.filter(s => s.is_read).length;
    return {
      ...bc,
      readCount: read,
      totalCount: total,
            ...
            <CardContent className="p-6">
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                <div className="space-y-2 flex-1">
                  ...
                  <p className="text-slate-400 text-sm line-clamp-1 opacity-70">{bc.message}</p>
                </div>
                ...
                <div className="flex flex-wrap items-center gap-8 md:px-8 border-l border-slate-800/50">
                    ...
                    <div className="text-center">
                        <div className="text-2xl font-black text-slate-700">/ {bc.recipient_count || bc.totalCount}</div>
