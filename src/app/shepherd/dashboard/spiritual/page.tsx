...
const TOOLTIP_STYLE = {
    ...
    labelStyle: { color: 'rgba(255,255,255,0.3)', fontSize: 9, textTransform: 'uppercase' as const },
};
...
export default function SpiritualPage() {
    ...
    const COLORS = ['#8b5cf6', '#06b6d4', '#f87171', '#34d399', '#fbbf24'];
    ...
    return (
        <div className="p-6 xl:p-8">
            <div className="mb-6">
                <h1 className="text-xl font-black text-white">Spiritual Analytics</h1>
                <p className="text-[11px] text-white/30 mt-0.5">Devotion tracking, streaks, SOAP journal insights</p>
            </div>

            {/* KPIs */}
            <div className="grid grid-cols-4 gap-3 mb-6">
                {[
                    { label: 'Avg Streak', val: `${avgStreak}d`, icon: Flame, color: 'text-amber-400' },
                    { label: 'Active Today', val: activeToday, icon: HeartPulse, color: 'text-emerald-400' },
                    { label: 'Total Devotions', val: totalDevotions, icon: BookOpen, color: 'text-violet-400' },
                    { label: 'SOAP Entries', val: soap.length, icon: BarChart2, color: 'text-cyan-400' },
                ].map(k => (
                    <div key={k.label} className="bg-[#111827] border border-white/5 rounded-2xl p-4">
                        <div className="flex items-center justify-between mb-2">
                            <p className="text-[9px] font-black text-white/30 uppercase tracking-widest">{k.label}</p>
                            <k.icon className={`w-4 h-4 ${k.color}`} />
                        </div>
                        <p className="text-2xl font-black text-white">{loading ? '—' : k.val}</p>
                    </div>
                ))}
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                ...
                <div className="bg-[#111827] border border-white/5 rounded-2xl p-5">
                    <p className="text-xs font-black text-white/40 uppercase tracking-widest mb-4">Weekly Completion Trend</p>
                ...
                {/* SOAP Sentiment */}
                <div className="bg-[#111827] border border-white/5 rounded-2xl p-5">
                    <p className="text-xs font-black text-white/40 uppercase tracking-widest mb-2">SOAP Journal Themes</p>
            ...
            {/* Streak Leaderboard */}
            <div className="bg-[#111827] border border-white/5 rounded-2xl p-5">
                <p className="text-xs font-black text-white/40 uppercase tracking-widest mb-4">Streak Leaderboard</p>
                {loading ? <div className="text-center py-8 text-white/30 text-xs">Loading...</div> : (
                    <div className="space-y-2">
                        {topStreaks.map((s, i) => (
                            <div key={s.id || i} className="flex items-center gap-3">
                                <span className={`text-[10px] font-black w-5 text-center ${i < 3 ? 'text-amber-400' : 'text-white/20'}`}>{i + 1}</span>
                                <div className="w-7 h-7 rounded-lg bg-violet-500/20 flex items-center justify-center text-xs font-black text-violet-400">
                                    {(s.name || 'U')[0]?.toUpperCase()}
                                </div>
                                <div className="flex-1 min-w-0">
                                    <p className="text-xs font-bold text-white">{s.name || `Member #${i + 1}`}</p>
                                ...
                                <div className="flex items-center gap-1">
                                    ...
                                    <p className="text-xs font-black text-white">{s.current_streak || 0}d</p>
                                </div>
                            </div>
                        ))}
                        {topStreaks.length === 0 && <p className="text-center text-white/25 text-xs py-4">No streak data yet</p>}
                    </div>
                )}
            </div>
        </div>
