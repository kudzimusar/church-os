...
export default function PublicNav() {
  ...
  const navLinks = [
    { label: 'WATCH', href: '/jkc-devotion-app/welcome#watch' },
    { label: 'VISIT', href: '/jkc-devotion-app/welcome#visit' },
    { label: 'GIVE', href: '/jkc-devotion-app/welcome#give' },
  ];
    ...
    <>
      <nav className="fixed top-0 w-full z-50 h-16 bg-black/80 backdrop-blur-xl border-b border-white/10">
        <div className="max-w-screen-xl mx-auto px-6 h-full flex items-center justify-between">
          {/* Left side: Logo */}
          <Link href="/jkc-devotion-app/welcome">
            <img 
        ...
        <div className="fixed inset-0 z-[60] bg-black flex flex-col items-center justify-center space-y-8 animate-in fade-in zoom-in duration-300">
          ...
          {navLinks.map((link) => (
            <Link 
              key={link.label} 
              href={link.href}
              className="text-2xl font-black tracking-[0.2em] text-white/70 hover:text-white transition-colors"
              onClick={() => setIsMenuOpen(false)}
            >
              {link.label}
            </Link>
