    ...
    const loadData = useCallback(async () => {
        ...
        try {
            ...
            const [
                ...
                db.from('vw_attendance_reconciliation').select('*').eq('org_id', orgId).order('event_date', { ascending: false }).limit(1),
            ...
            setData(prev => ({
                ...
                activeToday,
                avgStreak,
                lastSundayAttendance: recon?.usher_headcount || prev.lastSundayAttendance,
                weeklyAvgAttendance: Math.round((recon?.usher_headcount || prev.lastSundayAttendance) * 0.95),
